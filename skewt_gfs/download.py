...
def selection(cfg):
    return {"bounds": cfg.bounds, "levels": cfg.levels,
            "variables": ["TMP", "HGT", "UGRD", "VGRD", "RH", "PRES", "DPT"],
            "extra_levels": ["surface", "2_m_above_ground", "10_m_above_ground"]}


def build_url(cfg, cycle: datetime, hour: int):
    ...
    params = {"file": f"gfs.t{cycle:%H}z.pgrb2.0p25.f{hour:03d}",
              **{f"lev_{p}_mb": "on" for p in cfg.levels},
              **{f"var_{v}": "on" for v in selection(cfg)["variables"]},
              **{f"lev_{v}": "on" for v in selection(cfg)["extra_levels"]},
...
class Downloader:
    ...
    def _get(self, url, path=None):
        ...
        started = self.clock()
        request = Request(url, headers={"User-Agent": "skewt-gfs/1.0 regional sounding application"})
        try:
    ...
    def available(self, cycle, hour):
        url = f"{DIRECT}/gfs.{cycle:%Y%m%d}/{cycle:%H}/atmos/gfs.t{cycle:%H}z.pgrb2.0p25.f{hour:03d}.idx"
    ...
    def download(self, cycle, hour):
        ...
        url = build_url(self.cfg, cycle, hour)
